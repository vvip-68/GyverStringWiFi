...
#### Основные возмиожности пректа:
 - Поддержка контроллера с большим объемом памятии наличием WiFi на борту - NodeMCU или Wemos D1
 - Кнопка управления для включения/выключения устройства, изменения яркости
 - Управление устройством - через WiFi (локальная сеть)
 - Синхронизация времени с NTP сервером через интернет
 - Программа управления устройством на Andrioid
 - Пять эффектов раскраски текста бегущей строки
